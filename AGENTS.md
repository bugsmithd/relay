...
`/Users/divyanshurathore/dev/personal/relay/.planning/Codex-slack-agent-gates-week1-grounded-20260509.md`
...
Fresh app repo at `/Users/divyanshurathore/dev/personal/relay`. As of this cleanup: `AGENTS.md`, `.planning/Codex-slack-agent-gates-week1-grounded-20260509.md`, and `docs/decisions/backend.md` exist; `.omx/` runtime state and `.Codex/settings.local.json` remain outside the repo. No `git init`, no `package.json`, no `Makefile`, no migrations, no tests. The first Day 1A action is project init.
...
When editing these, expect a paired Codex review report + transcript (Day 2B+) and a non-`BLOCK` verdict for the change to pass `scripts/check-evidence.mjs`.
...
- Do not fabricate manifest entries, SHA256 values, Codex transcripts, or citation lines.
...
The PreToolUse protected-file hook (`.Codex/settings.json`) lands Day 2B and reads `evidence/trust-boundary-paths.json`. Until then, use the list above.

## Reviewer provenance

When asked to review code:

- Before Day 2B exists: do read-only inline review with concrete file paths and line numbers. Do not produce `Codex-authz-review.json` files; that runner does not exist yet.
- Day 2B onward: reviews go through `scripts/run-Codex-review.mjs`, which writes paired `evidence/runs/<run-id>/Codex-authz-review.json` and `Codex-authz-review-transcript.jsonl`. Hand-written review JSON is rejected by `scripts/check-evidence.mjs`.
