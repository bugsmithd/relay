...
8. **Tests** (plan §221-228, 8 specs).
   ...
   - **`tests/rls/policy-shape.spec.ts`** — DB-backed, admin client. Queries `pg_policies` for `tablename in ('channels','channel_members','messages')`. Asserts:
     - `messages` SELECT policy `qual` (the policy expression) contains the substring `auth.uid()` AND the substring `channel_members`.
     - `messages` INSERT policy `with_check` contains both substrings.
     - `channels` SELECT policy `qual` contains both substrings.
     - No row in `pg_policies` has `cmd in ('UPDATE','DELETE')` for any of the three tables.
     Failure shape: the test fails if a future migration adds an UPDATE/DELETE policy to any of the three tables.
...
4. `node --conditions=react-server --test tests/rls/policy-shape.spec.ts` exits 0:
   - `messages` SELECT policy expression references `auth.uid()` and `channel_members`.
   - `messages` INSERT policy `with_check` references `auth.uid()` and `channel_members`.
   - `channels` SELECT policy expression references `auth.uid()` and `channel_members`.
   - No UPDATE/DELETE policies exist for any of the three tables.
...
3. **Policy-shape test as the false-pass canary.** `policy-shape.spec.ts` queries `pg_policies` for substring matches against `qual` / `with_check`. A regex matching `auth.uid()` in a comment instead of an SQL token would false-pass — anchor assertions against `pg_policies` row content, not raw migration SQL.
